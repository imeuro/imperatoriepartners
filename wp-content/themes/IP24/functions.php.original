...
function ip24_setup() {
	/*
		...
		* If you're building a theme based on IP24, use a find and replace
		* to change 'ip24' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'ip24', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'ip24' ),
		)
	);
...
add_action( 'after_setup_theme', 'ip24_setup' );
 ...
 */
function ip24_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'ip24_content_width', 640 );
}
add_action( 'after_setup_theme', 'ip24_content_width', 0 );
 ...
 */
/*
function ip24_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'ip24' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'ip24' ),
...
add_action( 'widgets_init', 'ip24_widgets_init' );
*/
